...
async function read_prom() {
   ...
   // Функции fs.readdir() и fs.readFile(), использующие колбэки, оборачиваем функцией, которая позволит использовать промисы.
   ...
   // Ждём завершения всех промисов, добавленых в массив при вызове метода map.
   Promise.all(
